...
const AP_Param::GroupInfo AP_CANopen::var_info[] = {
    ...
    AP_GROUPINFO("NPOLE", 1, AP_CANopen, _num_poles, DEFAULT_NUM_POLES),
...
AP_CANopen::AP_CANopen()
{
    ...
    if (_singleton != nullptr) {
        AP_HAL::panic("AP_CANopen must be singleton");
    }
#endif
    _singleton = this;
}

void AP_CANopen::init()
{
    if (_driver != nullptr) {
    ...
    for (uint8_t i = 0; i < HAL_NUM_CAN_IFACES; i++) {
        if (CANSensor::get_driver_type(i) == AP_CAN::Protocol::CANopen) {
            _driver = new AP_CANopen_Driver();
            return;
        }
    }
}
...
AP_CANopen_Driver::AP_CANopen_Driver() : CANSensor("CANopen")
{
    register_driver(AP_CAN::Protocol::CANopen);

    // start thread for receiving and sending CAN frames. Tests show we use about 640 bytes of stack
    hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AP_CANopen_Driver::loop, void), "canopen", 2048, AP_HAL::Scheduler::PRIORITY_CAN, 0);
}

// parse inbound frames
void AP_CANopen_Driver::handle_frame(AP_HAL::CANFrame &frame)
{
    if (!frame.isExtended()) {
        return;
    }
    ...
    if (id.object_address != TELEMETRY_OBJ_ADDR) {
        GCS_SEND_TEXT(MAV_SEVERITY_DEBUG,"CANopen: rx id:%d, src:%d, dest:%d, len:%d", (int)id.object_address, (int)id.source_id, (int)id.destination_id, (int)frame.dlc);
    }
#endif
    ...
    switch (id.object_address) {
        case ESC_INFO_OBJ_ADDR:
            ...
            {
                if (__builtin_popcount(_init.detected_bitmask) >= CANOPEN_MAX_NUM_ESCS) {
                ...
                if ((bitmask & _init.detected_bitmask) != bitmask) {
                    ...
                    GCS_SEND_TEXT(MAV_SEVERITY_INFO,"CANopen: Found ESC id %u mapped to SERVO%u", id.source_id, id.source_id-1);
                }
            }
        break;
...
void AP_CANopen_Driver::update(const uint8_t num_poles)
{
...
bool AP_CANopen_Driver::send_packet_uint16(const uint8_t address, const uint8_t dest_id, const uint32_t timeout_ms, const uint16_t data)
{
    const uint16_t data_be16 = htobe16(data);
    return send_packet(address, dest_id, timeout_ms, (uint8_t*)&data_be16, 2);
}

bool AP_CANopen_Driver::send_packet(const uint8_t address, const uint8_t dest_id, const uint32_t timeout_ms, const uint8_t *data, const uint8_t data_len)
{
...
// singleton instance
AP_CANopen *AP_CANopen::_singleton;

namespace AP {
AP_CANopen *canopen()
{
    return AP_CANopen::get_singleton();
}
};
